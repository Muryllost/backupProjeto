...
E: ${questoes.alternativa_e}`;
  ...
  else if (questoes.correta === "alternativa_e")
    textoCorreto = questoes.alternativa_e;
  ...
  deleteButton.addEventListener("click", async () => {
    try {
      const response = await fetch(
        `http://localhost:3000/perguntas/${questoes.id_pergunta}`,
        {
          method: "DELETE",
        }
      );
  ...
  editbutton.addEventListener("click", () => {
    ...
    document.getElementById("edit-e").value = questoes.alternativa_e;
    document.getElementById("edit-correta").value = questoes.correta;

    document
      .getElementById("salvar-edicao")
      .setAttribute("data-id", questoes.id_pergunta);
...
document
  ...
  .addEventListener("click", async (event) => {
    ...
    const atualizado = {
      ...
      alternativa_e: document.getElementById("edit-e").value.trim(),
      correta: document.getElementById("edit-correta").value,
    };
    try {
      const response = await fetch(
        `http://localhost:3000/perguntas/${id_pergunta}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
